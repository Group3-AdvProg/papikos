...
@SpringBootTest
@ContextConfiguration(classes = {WishlistServiceImplTest.AsyncTestConfig.class})
class WishlistServiceImplTest {
    ...
    void testAddToWishlistSuccess() throws Exception {
        ...
        // Wait for async observer registration to complete
        CompletableFuture<Void> future = wishlistService.createObserverAsync(houseId, tenantId);
        future.get();
    ...
    @Test
    void testAsyncObserverRegistration() throws Exception {
        // Test that the async observer registration completes successfully
        CompletableFuture<Void> future = wishlistService.createObserverAsync(houseId, tenantId);
        assertDoesNotThrow(future::get);
        verify(notifier, times(1)).registerObserver(eq(houseId), any());
    }
