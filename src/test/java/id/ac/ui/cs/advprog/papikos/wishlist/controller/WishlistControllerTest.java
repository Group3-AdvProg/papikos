...
public class WishlistControllerTest {
    ...
    void testAddToWishlist() throws Exception {
        ...
        mockMvc.perform(post("http://localhost:8080/api/wishlist/add")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated());

        verify(wishlistService).addToWishlist("tenant123", 101L);  // Verify the service method call
    }
    ...
    void testGetWishlistByTenant() throws Exception {
        ...
        mockMvc.perform(get("http://localhost:8080/api/wishlist/tenant/tenant123"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(2))
                .andExpect(jsonPath("$[0]").value(101))
                .andExpect(jsonPath("$[1]").value(102));

        verify(wishlistService).getWishlistByTenant("tenant123");  // Verify the service method call
    }
    ...
    void testGetNotificationsByTenant() throws Exception {
        ...
        mockMvc.perform(get("http://localhost:8080/api/wishlist/notifications/tenant123"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1))
                .andExpect(jsonPath("$[0]").value("Room with ID 101 is now available!"));

        verify(wishlistService).getNotificationsByTenant("tenant123");  // Verify the service method call
    }
}
